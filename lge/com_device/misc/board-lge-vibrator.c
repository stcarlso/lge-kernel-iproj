...
//			printk(KERN_INFO "LGE: %s D_VAL = 0x%X %d\n", __func__,D_VAL,D_VAL);
	...
	for(val_cnt =0; val_cnt<3;val_cnt++)
	{
		...
		{
			printk(KERN_INFO "LGE_ON: %s[val_cnt = %d]  Read GP Mn MDIV reg val 0x%X\n", __func__, val_cnt , reg_val);
			writel(0x0002, vib_base_ptr + GP_MN_CLK_MDIV_REG );
		}
	}
	...
	for(val_cnt =0; val_cnt<3;val_cnt++)
	{
		...
		{
			printk(KERN_INFO "LGE_ON: %s[val_cnt = %d]  Read GP Mn NDIV reg val 0x%X\n", __func__, val_cnt , reg_val);
			writel(0x1AE9, vib_base_ptr + GP_MN_CLK_NDIV_REG);
		}
	}
